...
export const EnhancedCard = forwardRef<HTMLDivElement, EnhancedCardProps>(
  ({ variant = 'default', padding = 'md', animate = true, hover = true, className = '', children }, ref) => {
    ...
    const classes = `${variantClasses[variant]} ${paddingClasses[padding]} border ${theme.borderColors.primary} rounded-xl ${
      hover && variant === 'interactive' ? 'hover:' + theme.backgrounds.cardHover + ' cursor-pointer' : ''
    } ${className}`;
...
export const EnhancedButton = forwardRef<HTMLButtonElement, EnhancedButtonProps>(
  ({ 
    ...
    const classes = `${variantClasses[variant]} ${sizeClasses[size]} rounded-lg font-semibold transition-all duration-200 ${theme.interactive.focus} ${
...
export const EnhancedInput = forwardRef<HTMLInputElement, EnhancedInputProps>(
  ({ 
    ...
    } rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all duration-200 ${
      disabled ? 'opacity-50 cursor-not-allowed' : ''
    }`;
