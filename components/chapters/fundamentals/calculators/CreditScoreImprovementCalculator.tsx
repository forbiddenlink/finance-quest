    ...
    <div className={`max-w-7xl mx-auto ${theme.backgrounds.glass} border ${theme.borderColors.primary} rounded-lg shadow-lg p-6`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className={`${theme.status.info.bg} border ${theme.status.info.border} rounded-lg p-6 text-center`}>
          <h3 className={`font-semibold ${theme.status.info.text} mb-3 flex items-center justify-center gap-2`}>
            <Shield className="w-5 h-5" />
            Current Estimated Score
          </h3>
          <div className={`text-4xl font-bold ${getScoreColor(currentEstimated)} mb-2`}>
            {currentEstimated}
          </div>
        ...
        <div className={`${theme.status.success.bg} border ${theme.status.success.border} rounded-lg p-6 text-center`}>
          <h3 className={`font-semibold ${theme.status.success.text} mb-3 flex items-center justify-center gap-2`}>
            <Target className="w-5 h-5" />
            Target Score Potential
          </h3>
          <div className={`text-4xl font-bold ${getScoreColor(targetEstimated)} mb-2`}>
            {targetEstimated}
          </div>
          <p className={`${theme.textColors.secondary} text-sm`}>
            {getScoreLevel(targetEstimated)} Credit
          </p>
          <div className={`mt-3 text-lg font-semibold ${theme.status.success.text}`}>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-4">
          {creditFactors.map((factor) => (
            <div key={factor.id} className={`${theme.backgrounds.card} border ${theme.borderColors.primary} rounded-lg p-4`}>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Factor Info */}
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`${getImpactColor(factor.impact)} p-2 rounded-lg`}>
                      <Star className="w-4 h-4 text-white" />
                    </div>
                    <div>
                      <h4 className={`font-semibold ${theme.textColors.primary}`}>{factor.name}</h4>
                      <p className={`text-sm ${theme.textColors.secondary}`}>{factor.weight}% of score</p>
                    </div>
                  </div>
                ...
                {/* Action Steps */}
                <div>
                  <h5 className={`font-medium ${theme.textColors.primary} mb-2 flex items-center gap-2`}>
                  ...
                  <ul className="space-y-1">
                    {factor.actionSteps.map((step, index) => (
                      <li key={index} className={`text-sm ${theme.textColors.secondary} flex items-start gap-2`}>
                        <span className={`w-4 h-4 ${theme.status.info.bg} rounded-full flex items-center justify-center flex-shrink-0 mt-0.5`}>
                          <span className={`w-1.5 h-1.5 ${theme.status.info.text} rounded-full`}></span>
                        </span>
                        {step}
                      </li>
                    ))}
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {scenarios.map((scenario, index) => (
            <div key={index} className={`${theme.backgrounds.card} border ${theme.borderColors.primary} rounded-lg p-4`}>
              <div className="text-center mb-4">
                <h4 className={`font-semibold ${theme.textColors.primary} mb-2`}>{scenario.timeframe}</h4>
                <div className={`text-3xl font-bold ${getScoreColor(scenario.estimatedScore)} mb-1`}>
              ...
              <div className="space-y-3">
                <div>
                  <h5 className={`text-sm font-medium ${theme.textColors.primary} mb-2`}>Key Actions:</h5>
                  <ul className="space-y-1">
                    {scenario.actions.slice(0, 2).map((action, i) => (
                      <li key={i} className={`text-xs ${theme.textColors.secondary} flex items-start gap-1`}>
                ...
                <div>
                  <h5 className={`text-sm font-medium ${theme.textColors.primary} mb-2`}>Benefits:</h5>
                  <ul className="space-y-1">
                    {scenario.benefits.slice(0, 2).map((benefit, i) => (
                      <li key={i} className={`text-xs ${theme.textColors.secondary} flex items-start gap-1`}>
                        <Award className="w-3 h-3 text-blue-400 flex-shrink-0 mt-0.5" />
                        {benefit}
                      </li>
                    ))}
                  </ul>
                ...
                <div className={`${theme.status.success.bg} border ${theme.status.success.border} rounded p-2 text-center`}>
                  <p className={`text-xs ${theme.status.success.text} mb-1`}>Est. Monthly Savings</p>
